...
class Typewriter {
    ...
    tick() {
        ...
        this.text = this.isDeleting? fullText.substring(0, this.text.length - 1) : fullText.substring(0, this.text.length + 1);
...
window.onload = () => {
    ...
    let rotate, period;
    for (let elt of elts) {
        rotate = elt.getAttribute('data-type');
        period = elt.getAttribute('data-period');
        if (rotate) new Typewriter(elt, JSON.parse(rotate), period);
    }
